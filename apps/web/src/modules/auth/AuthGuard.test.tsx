...
describe("AuthorGuard", () => {
  ...
  it("keeps the author workspace role-gated instead of eligibility-gated", () => {
    mockUseAuth.mockReturnValue({
    ...
    mockUseMarketplaceProfile.mockReturnValue({
      data: {
        profile: { eligibilityStatus: "blocked", role: "author" },
      },
      isPending: false,
    });
    ...
    expect(markup).toContain("author-only content");
    expect(markup).not.toContain("manuscripts.forbidden.title");
  });
});
