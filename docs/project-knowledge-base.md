...
- Recently changed for Step 9: scanner posture is now explicit. Local/dev can use `DOCUMENT_SCANNER_MODE=local_fake` and `scanner_result = not_scanned`; staging/production fail fast unless they configure a real scanner provider or name `DOCUMENT_SCANNER_LAUNCH_DECISION_ID`.
...
- Next recommended step: verify the full local upload -> queued job -> processor dispatch -> processed/failed document loop end to end against the local Supabase project, then add the real document scanner integration slice before real staging/production user uploads.
- Known blockers: actual GCP bucket provisioning/IAM, production Cloud Tasks/private Cloud Run invocation, embedding model choice, real malware/scanner provider or explicit launch decision, production prices and quotas, account deletion flow, Resend domain verification and secret entry, Sentry project creation/secret entry, and remote GitHub branch protection enforcement are still open.
