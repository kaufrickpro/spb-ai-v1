...
- Current phase: Step 11 intro requests are implemented for GitHub issues #67-#74 on top of the Step 10 repository-backed matching slice. Users can send pair-scoped intro requests from stored match/access evidence, manage pending requests, enforce duplicate/terminal/cooldown/quota rules, unlock accepted-intro contact/sample access, and let admins inspect safe read-only intro timelines. Step 9 and Step 10 foundations remain in place; live remote validation still depends on applying the existing Step 10 migrations plus the new Step 11 migration and configured Vertex/Gemini credentials for matching.
- Recently changed for Step 11 / GitHub issues #67-#74: added `intro_requests`, `notifications`, `product_audit_events`, `intro_request_usage_events`, `public.has_accepted_intro(...)`, and service-role RPCs for transactional create/accept/reject/cancel. Existing remote databases must apply `supabase/migrations/20260508213000_step11_intro_requests.sql`.
- Recently changed for Step 11 / GitHub issues #67-#74: intro creation accepts only `{ manuscriptId, publisherProfileId, message? }`, rejects forged fields, derives requester/recipient/author from trusted state, requires stored match candidate or approved manuscript access evidence, requires currently eligible profiles/manuscript/sample, and records redacted notifications/product audit events.
- Recently changed for Step 11 / GitHub issues #68-#73: `/app/requests` now lists intro requests beside manuscript access requests, supports recipient accept/reject and requester cancel, uses API-owned `introState`, and shows accepted-intro contact separately from match-visible contact. Publisher sample download remains an explicit signed-URL click after acceptance.
...
- Recently fixed for local Step 10 matching smoke: added a forward migration granting `authenticated` read privileges and `service_role` management privileges on Step 10 matching/access tables so `/app/matches`, run history, and API-owned match run creation can access `match_runs` and related tables through Supabase PostgREST.
...
- Next recommended step: apply/verify the existing Step 10 remote migrations plus `supabase/migrations/20260508213000_step11_intro_requests.sql` in staging, configure the AI-service Vertex/Gemini env contract, run a staging match and intro-request smoke against the PSC Vector Search endpoint, and then continue billing/usage hardening.
