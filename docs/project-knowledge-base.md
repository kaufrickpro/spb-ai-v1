...
- Recently changed for Step 12 / GitHub issues #75-#82: match cards now show compact previews only, while the candidate detail page owns comparison, axis evidence, snippets, watch-outs, limitations, and stale-run messaging with a safe link to profile history for rematch. Accepted-intro contact and sample download controls remain off match detail.
