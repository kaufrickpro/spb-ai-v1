...
- Current phase: Step 10 matching is implemented through GitHub issues #60-#65 as a repository-backed AI-service matching slice. Step 10 Phase 0 remains implemented for GitHub issues #40-#45, and the durable run/profile-access lifecycle from #47-#51 remains the run foundation. Step 9 remains complete for ingestion/scanner foundations. Staging/production AI service config now fails closed unless Vertex embeddings, Vertex Vector Search, PSC/query routing, and Vertex/Gemini explanations are configured. The deployed API matching path creates runs, calls the AI service with only `{ match_run_id }`, reads AI-persisted candidates, and leaves failed AI results failed with zero candidates. The AI service now owns signal sync, Vertex adapter boundaries, retrieval/scoring/ranking, top-25 candidate persistence, profile-access grant persistence, and top-10 explanation persistence; live remote validation still depends on applying existing Step 10 migrations and configured Vertex/Gemini credentials.
- Recently planned for Step 11: intro requests are pair-scoped to one manuscript and one publisher profile. Either side can initiate only when stored match/access evidence exists, such as a stored match candidate or an approved manuscript access request. Public directory browsing, guessed IDs, and unrelated profile access must not permit outreach.
- Recently planned for Step 11: duplicate pending intro requests are blocked per pair; accepted is terminal; rejected or cancelled pairs can retry after a 14-day cooldown. Quota is consumed when the request is sent, transactionally with request creation, notification creation, and product audit event creation. The starter limit is 10 intro requests per user/day until Step 13 wires real subscription plan limits.
- Recently planned for Step 11: accepted intro access is computed live from an accepted `intro_requests` row plus current eligibility. Contact unlock is mutual through an `acceptedIntroContact` API response block; sample unlock is publisher-only and manuscript-specific through the existing signed download URL path. Accepted intro does not unlock other manuscripts, full manuscript text, document chunks, admin notes, billing state, or broad direct table access.
- Recently planned for Step 11: in-app notification rows and `product_audit_events` are introduced for intro lifecycle events. Product email remains deferred to Step 14. `admin_audit_logs` stays focused on admin/moderation actions.
- Recently planned for Step 11: admin gets a read-only intro request investigation surface with filters, safe metadata, timeline, and current unlock status. Admins do not accept, reject, cancel, or impersonate users in Step 11.
- Recently planned for Step 11: the detailed implementation plan lives in `docs/step-11-intro-requests-implementation-plan.md`. Step 11 can proceed on stored match/access evidence before final AI-service scoring persistence, but the current recommended implementation path still prioritizes the repository-backed Step 10 AI-service matching worker unless product priorities change.
...
- Recently changed for Step 10 / GitHub issues #60-#64: added Vertex text embedding, Vector Search datapoint upsert, and Vector Search `findNeighbors` adapters with metadata-token auth, safe provider failures, mocked request/response tests, and no numeric vector persistence in Supabase records.
- Recently changed for Step 10 / GitHub issues #62-#64: added AI-service signal sync and matching workers. The sync worker embeds current eligible manuscript/publisher signals and records missing optional publisher wishlist/catalog signals. The match worker loads trusted runs, builds/upserts source and candidate signals, retrieves top neighbors per axis, merges duplicate candidates, applies three-axis soft scoring, hides scores below `0.35`, persists up to 25 candidates and match profile-access grants, and requires valid Gemini paragraphs for ranks 1-10 before candidate insertion.
...
- Recently changed for Step 10 / GitHub issue #65: the deployed API matching path no longer calls API-owned tracer candidate persistence. After the AI service returns success, the API counts and reads `match_candidates` rows already persisted by the AI service; AI timeout/failure results mark the run failed with zero candidates. Local test auth still uses in-memory tracer-like fixtures.
- Recently changed for Step 10 / GitHub issues #60-#64: the Vertex/Gemini explanation provider is now connected to the repository-backed matching worker. Gemini receives bounded rank-1-through-10 evidence only; malformed, missing, unsafe, or provider-failed output fails the run before candidate insertion. Ranks 11-25 persist with `explanation_status = not_requested`.
...
- Recently changed for Step 10 / GitHub issues #47-#51 and #65: added shared matching contracts/OpenAPI, `POST /api/v1/matches/run`, `GET /api/v1/matches`, `GET /api/v1/profile/history`, run detail, and candidate detail routes. Supabase-backed author-to-publisher and publisher-to-manuscript runs now create stored runs, enforce role/eligibility/sample/rate-limit gates, call the private AI service with `{ match_run_id }`, and read AI-persisted match candidates.
...
- Recently decided for production setup: product name is Smart Publishing Bridge; canonical production URL is `https://spb-ai.com`; `https://www.spb-ai.com` redirects to the apex domain; staging uses `https://spb-ai.dev`.
- Recently decided for production setup: auth callbacks are `https://spb-ai.com/auth/callback` for production and `https://spb-ai.dev/auth/callback` for staging. Supabase Auth Site URL and allowed redirect URL settings must use those environment-specific app URLs.
...
- Recently decided for production setup: Supabase Auth lifecycle emails use Resend SMTP from `no-reply@auth.spb-ai.com` in production and `no-reply@auth.spb-ai.dev` in staging. API-owned product email uses `support@mail.spb-ai.com` in production and `support@mail.spb-ai.dev` in staging.
...
- Recently changed for real Sentry wiring: the React frontend, Fastify API, and FastAPI AI service now initialize official Sentry SDKs when DSNs are present, carry environment/release/sample-rate config, and run centralized before-send scrubbing for manuscript text, document chunks, signed URLs, secrets, payment/email signatures, and unreleased contact details. The frontend adds a Sentry error boundary and React Router v7 browser tracing; the API captures unhandled route errors through the shared Fastify error handler; the AI service enables FastAPI/httpx integrations.
- Recently changed for real scanner setup: added the repo-owned `apps/document-scanner` ClamAV HTTP scanner deployable plus staging deploy script. Staging env templates now default to real `http-clamav` scanner mode, point scanner endpoint/token/timeout at Secret Manager entries, and support private Cloud Run OIDC through `DOCUMENT_SCANNER_CLOUD_RUN_AUDIENCE`.
...
- Next recommended step: apply/verify the existing Step 10 remote migrations in staging, configure the AI-service Vertex/Gemini env contract, run a staging match smoke against the PSC Vector Search endpoint, and then continue Step 11 intro request implementation.
...
- OAuth deployment rule: app callback URLs are environment-specific. Development uses `http://localhost:5173/auth/callback`; staging uses `https://spb-ai.dev/auth/callback`; production uses `https://spb-ai.com/auth/callback`. Frontend config, backend config, and Supabase Auth URL Configuration must use the matching environment URL before release testing.
