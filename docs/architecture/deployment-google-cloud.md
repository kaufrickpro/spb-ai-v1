...
- Staging AI-service config must set `STORAGE_PROVIDER=gcs`, `GCS_BUCKET_PRIVATE_UPLOADS=spb-ai-staging-manuscripts`, `SUPABASE_URL`, and `SUPABASE_SERVICE_ROLE_KEY`; the service reads private objects through its runtime service identity.
