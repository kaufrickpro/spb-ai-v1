...
- Local/dev may produce `scanner_result = not_scanned` or simulated scanner outcomes through the fake scanner adapter; this is developer/test evidence only, not a clean production scan. Staging and production may run with `not_scanned` only when `DOCUMENT_SCANNER_LAUNCH_DECISION_ID` names an explicit launch decision, and must not use fake clean/suspicious/quarantined outcomes.
- Create admin exceptions for suspicious scanner outcomes, quarantines, validation bypass signals such as file type mismatch, repeated system/provider failures after automatic retries including `scanner_failed`, or unexpected runtime errors. Suspicious scanner results enter Needs Review; quarantined scanner results enter Quarantine; repeated scanner provider failures enter System Failures. These exception records must store only safe operational fields such as failure code, attempt counts, job id, MIME type, `scanner`, `scanner_result`, `scanner_version`, bounded `scanner_signature`, and `scanner_error_type`; never store manuscript text, chunks, original filenames, signed URLs, private storage paths, author IDs, tokens, or raw scanner/provider payloads.
