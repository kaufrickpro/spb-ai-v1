...
begin
  ...
  where id = p_review_id
    and status = 'pending'
    and eligibility_status = 'limited'
    and review_outcome = 'needs_review'
  returning * into review_row;

  if not found then
    raise exception 'Pending review not found' using errcode = 'P0002';
  end if;
  ...
  if review_row.entity_type = 'profile' then
    ...
    if not found then
      raise exception 'Pending profile review not found' using errcode = 'P0002';
    end if;
